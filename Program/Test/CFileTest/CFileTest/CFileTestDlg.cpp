...
CCFileTestDlg::CCFileTestDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_CFILETEST_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CCFileTestDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CCFileTestDlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// CCFileTestDlg 消息处理程序
...
{
	// TODO: 在此添加控件通知处理程序代码
	std::string pathName = "D:\\info.txt";
	std::ofstream fout(pathName);
	if (fout) {
		fout << 1000 << "\t" << "fasfa" << std::endl << "test";
	}
